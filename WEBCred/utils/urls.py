    ...
    def __init__(self, lang_iso=None, ads_list=None):
        if lang_iso:
            try:
                ...
                print("Error messagagagagaga",str(e),e.message)
...
class Urlattributes(object):
    ...
    try:
        ...
        if not normalizedData:
            ...
            data = db.getdbdata() # this makes the data empty . database is empty.
            #print("DB",data)

            it = normalizeCategory['3'].items()
            #it=list(normalizeCategory['3'].items())
            #print(it)
            for k in it:
             #   print(k)
              #  print(k[0])
            ...
            it=list(normalizeCategory['misc'])[0]
